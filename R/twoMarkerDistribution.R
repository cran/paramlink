...
twoMarkerDistribution <- function(x, id, partialmarker1, partialmarker2, theta, loop_breakers=NULL, eliminate=99, verbose=TRUE) {
    ...
    grid.subset = fast.grid(c(geno.grid.subset(x, m1, id, "AUTOSOMAL", make.grid=F), geno.grid.subset(x, m2, id, "AUTOSOMAL", make.grid=F))) #Do this before loop breaking, since eliminate2 works better WITH the loops.
