linkageSim <- function(x, N=1, available=x$available, afreq=NULL, partialmarker=NULL, 
   ...
   likel_counter = 0
	...
	nInd = x$nInd; chrom = x$model$chrom; nall = length(afreq);
	if(length(available) == 0) 
      available = x$orig.ids
   sim_origs = available
   
   if (nall > 4) stop("Sorry - simulation of markers with more than 4 alleles is not implemented yet.")
   ...
   #initz = colSums(initialCalc$initial_probs[, sim_indivs, drop=F] == 0)
   ...
   genoprobs <- function(x, partial, id, values) {  
      ...
      probs = unlist(lapply(values, function(g) { 
         part = partial # to avoid NOTE is R CMD check
         part[id] = g  
   ...
   markerdata_list = lapply(2*seq_len(ncol(markers)), function(k) {mk = markers2[, c(k-1, k)];	attributes(mk) = attrib; mk})
