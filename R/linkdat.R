...
function(ped, model=NULL, map=NULL, dat=NULL, freq=NULL, annotations=NULL, missing=0, header=FALSE, checkped=TRUE, verbose=TRUE, ...) {  
    ...
    if (verbose) {
        if(nInd == 1) cat(sprintf("Singleton %s, individual ID = %d.\n", ifelse(pedigree[,'SEX']==1, 'male', 'female'), orig.ids))
        else cat(nInd, "individuals.\n")
        affs = sum(pedigree[, 'AFF'] == 2)
        if(affs > 0) cat(sprintf("%d affected, %d non-affected.\n", affs, nInd-affs))
    }
    ...
    if (nInd>1) {
        ...
        if(verbose) 
            if(hasLoops) cat("Loop(s) detected.\n")  
                ...
                cat(sprintf("%d nuclear %s.\n", length(subnucs), ifelse(length(subnucs)==1, "subfamily", "subfamilies")))
        class = 'linkdat'
    } else {
